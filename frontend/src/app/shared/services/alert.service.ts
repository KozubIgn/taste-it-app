...
import { AlertAlign, AlertDirection, AlertType } from "src/app/enums/alert.enum";
...
export class AlertService {
  ...
  public error(message: string, alertDirection: AlertDirection = AlertDirection.BOTTOM, alertAlign: AlertAlign = AlertAlign.RIGHT): void {
    this.prepareNotification(message, AlertType.ERROR, alertDirection, alertAlign, 'icon-simple-remove');
  }
  public primary(message: string, alertDirection: AlertDirection = AlertDirection.BOTTOM, alertAlign: AlertAlign = AlertAlign.RIGHT): void {
    this.prepareNotification(message, AlertType.PRIMARY, alertDirection, alertAlign);
  }

  public info(message: string, alertDirection: AlertDirection = AlertDirection.BOTTOM, alertAlign: AlertAlign = AlertAlign.RIGHT): void {
    this.prepareNotification(message, AlertType.INFO, alertDirection, alertAlign, 'icon-chat-33');
  }

  public warning(message: string, alertDirection: AlertDirection = AlertDirection.BOTTOM, alertAlign: AlertAlign = AlertAlign.RIGHT): void {
